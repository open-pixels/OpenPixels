...
<div class="stack-lg">
  ...
  {:else}
    <div class="card stack" data-testid="account-panel">
      <!--
        The panel's framing is ours. `heading`, `description` and `mark` are
        supported properties, which is how OpenPdfEdit does it -- far better
        than hiding the default header with injected shadow-DOM CSS, since
        that survives a bundle refresh and this does not depend on the
        element's internal class names. The default heading names a service
        the visitor has never heard of, above a request for a password, which
        reads exactly like the thing people are told to be suspicious of.

        No `return-to`: <openapps-login> declares no such property (only
        <openapps-buy> does), so setting one is silently ignored. The SDK's
        default is origin + pathname + search, and the code comes back to it
        in the *fragment* -- which App.svelte recognises and routes here.
      -->
      <openapps-login
        variant="panel"
        mark="✦"
